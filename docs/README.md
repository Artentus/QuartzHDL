...
To add a block to a match arm, use `{` and `}`. Such blocks must still be followed by a comma.
...
If statements are similar to Rust. They do not have parenthesis around the condition, and have if else and else branches.
