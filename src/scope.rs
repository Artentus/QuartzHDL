...
impl<'p> Scope<'p> {
    ...
    pub fn contains_func<'err>(&self, name: &'err Ident) -> QuartzResult<'err, usize> {
        if let Some(arg_count) = self.contains_func_exclusive(name) {
            ...
            Err(QuartzError::InvalidFuncIdent { name })
        } else {
            Err(QuartzError::UndefinedIdent { name })
        }
    }

    pub fn contains_const<'err>(&self, name: &'err Ident) -> QuartzResult<'err, ()> {
        if self.contains_const_exclusive(name) {
            Ok(())
        } else if self.contains_var_exclusive(name) {
            Err(QuartzError::ValueNotConst { name })
        } else if self.contains_func_exclusive(name).is_some() {
            Err(QuartzError::InvalidValueIdent { name })
        } else {
            Err(QuartzError::UndefinedIdent { name })
        }
    }

    pub fn contains_var<'err>(&self, name: &'err Ident) -> QuartzResult<'err, ()> {
    ...
    pub fn contains_mut_var<'err>(&self, name: &'err Ident) -> QuartzResult<'err, ()> {
        if self.contains_var_exclusive(name) {
            Ok(())
        } else if self.contains_const_exclusive(name) {
            Err(QuartzError::TargetNotAssignable { name })
        } else if self.contains_func_exclusive(name).is_some() {
            Err(QuartzError::InvalidValueIdent { name })
        } else {
            Err(QuartzError::UndefinedIdent { name })
